            ...
            primaryBlock.statements!!.iterator().forEachRemaining { statement ->
                run {
                    if (statement is SimpleDeclaration) {
                        if (statement.declSpecifier is CompositeTypeSpecifier && (statement.declSpecifier as CompositeTypeSpecifier).declarations != null ) {
                                ...
                                .forEachRemaining { decl ->
                                    run {
                                        if (decl is SimpleDeclaration && decl.declarators != null) {
                                                ...
                                                run {
                                                    if (declArr is Declarator) {
                                                        ...
                                                        if(declArr.name is Name) {
                                                            if (declaration is CPPASTSimpleDeclaration) {
                                                                if(declaration.declarators.size > 0) {
                                                                    if ((declArr.name as Name).name == declaration.declarators[0].name.rawSignature) {
                                                                        switch = true
                                                                    }
                                                                }
                                                            }
                                            ...
                                            decl.declarator!!.iterator().forEachRemaining { declarator ->
                                                run {
                                                    if (declarator is FunctionDeclarator) {
                                                        if (declarator.name is Name) {
                                                            if(declaration is CPPASTFunctionDefinition) {
                                                                if ((declarator.name as Name).name == declaration.declarator.name.rawSignature) {
                                                                    switch = true
                                                                }
                                                            }
                                                        }
                                ...
                                if (decl is FunctionDeclarator) {
                                    if(declaration is CPPASTFunctionDefinition) {
                                        if(decl.name is Name) {
                                            if ((decl.name as Name).name == declaration.declarator.name.rawSignature) {
                                                switch = true
                                            }
                                        } else if (decl.name is QualifiedName) {
                                            if((decl.name as QualifiedName).lastName is Name) {
                                                if(((decl.name as QualifiedName).lastName as Name).name == declaration.declarator.name.rawSignature) {
                                                    switch = true
                                                }
                                            }
                                        }
